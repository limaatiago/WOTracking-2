...
<div class="wrap">
	<h2>
		<?php echo get_admin_page_title() ; ?> <a href="admin.php?page=wot_novaos" class="add-new-h2">Cadastrar Nova</a>
	</h2>
	<div id="conteudo">
				...
				<tr>
					<td width="20%">
						<select name="tipoBusca" id="tipoBusca" style="width:100%;" required >
							...
							<option value="1" <?php if ( $buscar_ok ) { if ( $tipo_pesquisa == 1 ) { echo "selected"; } } ?>>Número de OS</option>
							<option value="2" <?php if ( $buscar_ok ) { if ( $tipo_pesquisa == 2 ) { echo "selected"; } } ?>>OS Externa</option>
							<option value="3" <?php if ( $buscar_ok ) { if ( $tipo_pesquisa == 3 ) { echo "selected"; } } ?>>CPF do Cliente</option>
							<option value="4" <?php if ( $buscar_ok ) { if ( $tipo_pesquisa == 4 ) { echo "selected"; } } ?>>Nome do Cliente</option>
						</select>
					</td>
					<td>
		...
		<table id="tabela-consulta" class="widefat">
			...
			if ( $resultados_os ) {
				...
				<tr>
					...
					elseif ( $resultados_os->status_os == 8 ) { echo "Produto entregue"; }
					elseif ( $resultados_os->status_os == 9 ) { echo "Rep. cliente. Entregue"; }
					elseif ( $resultados_os->status_os == 10 ) { echo "Rep. técnico. Entregue"; }
					else { echo "-"; }
					echo " </td>
					<td align='right'> 
